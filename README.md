...
2.  `plot_es()` — Plots event study results using `ggplot2` with
    ...
    theme customization.
3.  `plot_es_interactive()` — Creates interactive event study plots
    using `plotly` with hover tooltips displaying coefficients,
...
For staggered adoption designs with unit-varying treatment timing, use
`method = "sunab"` for the Sun & Abraham (2021) estimator:
