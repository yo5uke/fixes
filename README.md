...
> **Upgrading from < 1.0.0?** The verb-style API (`run_es()`, `calc_att()`,
> `run_did()`, `plot_es()`, ...) still works and returns identical results,
> but is deprecated. See `NEWS.md` for the old-to-new mapping.

**Estimators** (selected via the `estimator` argument in `event_study()` and `att()`):
...
Use `event_study()` with a fixed event date.  Here we use `fixest::base_did`,
...
![plot of chunk twm-plot](man/figures/README-twm-plot-1.png)
...
After estimating `event_study()` with a staggered estimator, `att()` computes
...
`plot()` works with results from any estimator. The default display is black
error bars; `type = "ribbon"` draws a line with a shaded band in the
package's muted steel blue.
...
![plot of chunk plot-ribbon](man/figures/README-plot-ribbon-1.png)

`color` sets the points (and the ribbon line), `fill` the band, and
`errorbar_color` recolours the bars on their own — it follows `color` unless
you set it.


``` r
plot(es, color = "#1B4965", errorbar_color = "grey60")
```

![plot of chunk plot-colours](man/figures/README-plot-colours-1.png)
...
``` r
plot(es, time_axis = "calendar")
```

![plot of chunk plot-calendar](man/figures/README-plot-calendar-1.png)
